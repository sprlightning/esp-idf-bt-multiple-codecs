...
bool a2dp_lc3plus_decoder_decode_packet(BT_HDR* p_buf, unsigned char* buf, size_t buf_len) {
    ...
    if (!ret) {
        ...
        if (nsamples > 0) {
            /* liblc3 emits LC3_PCM_FORMAT_S24: the 24-bit sample sits RIGHT-justified
             * in the low 3 bytes of each int32 (high byte sign-extended). Every other
             * codec in this sink reaches digital full-scale (16-bit <<16, LDAC packed
             * 24 <<8, aptX/LHDC left-justified), so a right-justified S24 would play
             * ~48 dB quieter. Left-shift <<8 here to MSB-align it, matching the
             * bits==32 passthrough in audio_sink_srv_data_output() -> uniform loudness. */
            size_t ntot = (size_t)nsamples * cb->channels;
            for (size_t i = 0; i < ntot; i++) pcm[i] <<= 8;
