                                    ...
                                    size_t buf_len)
{
  ...
  /* Decode straight into the framework's shared per-codec scratch buffer
   * (`buf`, sized BT_A2DP_SINK_BUF_AAC) instead of a private static buffer, so
   * AAC holds NO RAM when another codec is active. The Helix decoder writes one
   * frame per DecodeFrame call; we emit it immediately, then reuse `buf`. */
  INT_PCM* out = (INT_PCM*)buf;
  const int out_cap = (int)(buf_len / sizeof(INT_PCM));
  if (!out || out_cap <= 0) {
    LOG_ERROR("%s: no decode buffer (len=%u)", __func__, (unsigned)buf_len);
    return false;
  }
  ...
  while (bytesValid > 0) {
    ...
    while (true) {
      ...
      if (frame_len > buf_len) frame_len = buf_len;   /* never emit past the buffer */
      a2dp_aac_decoder_cb.decode_callback((uint8_t*)out, frame_len);
    }
  }
