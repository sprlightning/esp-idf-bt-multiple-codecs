 ...
 *
 * v5.1.4 适配：移除 v6.1.0 特有的 esp_bt_dev_cb_event_t / esp_bluedroid_init_with_cfg
 * / auth_cmpl.lk_type（v5.1.4 无这些 API）。
 */
...
void bredr_app_gap_evt_def_hdl(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    /* v5.1.4 适配：ESP_BT_GAP_ENC_CHG_EVT 为 v6.1.0 新增事件，v5.1.4 无此回调 */
    ...
    case ESP_BT_GAP_MODE_CHG_EVT:
        ESP_LOGI(BREDR_APP_TAG, "ESP_BT_GAP_MODE_CHG_EVT mode: %d",
                 param->mode_chg.mode);
        break;
...
esp_err_t bredr_app_common_init(void)
{
    ...
    /* v5.1.4：使用 esp_bluedroid_init()（无 cfg 参数） */
