...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
     ...
     * v5.1.4 多 codec 适配：用 codec_config 解析任意 A2DP codec
     * （SBC/AAC/aptX/aptX-HD/aptX-LL/LDAC/OPUS/LC3Plus/LHDC V5） */
    case ESP_A2D_AUDIO_CFG_EVT: {
        ...
        if (get_codec_config(a2d, &sample_rate, &bit_depth, &channels, &codec_type)) {
            ESP_LOGI(BT_AV_TAG, "Codec: %d, sample rate: %lu, bit depth: %u, channels: %u",
                     codec_type, (unsigned long)sample_rate, bit_depth, channels);
        } else {
            ESP_LOGE(BT_AV_TAG, "Unsupported codec type: %d, fallback to SBC defaults", a2d->audio_cfg.mcc.type);
            sample_rate = 44100;
            bit_depth = 16;
            channels = 2;
        }
        ...
        dac_continuous_config_t cont_cfg = {
            ...
            .offset = 127,
            .clk_src = DAC_DIGI_CLK_SRC_DEFAULT,   // Using APLL as clock source to get a wider frequency range
            .chan_mode = (channels == 1) ? DAC_CHANNEL_MODE_SIMUL : DAC_CHANNEL_MODE_ALTER,
        };
        /* Allocate continuous channels */
        ...
        i2s_std_slot_config_t slot_cfg = I2S_STD_MSB_SLOT_DEFAULT_CONFIG(bit_depth, channels);
        i2s_channel_reconfig_std_clock(tx_chan, &clk_cfg);
        i2s_channel_reconfig_std_slot(tx_chan, &slot_cfg);
        i2s_channel_enable(tx_chan);
    #endif
        ESP_LOGI(BT_AV_TAG, "Audio player configured, sample rate: %lu, bit depth: %u, channels: %u",
                 (unsigned long)sample_rate, bit_depth, channels);
        break;
    }
